...
import { clearSpringBones, loadSpringBonesFailure, loadSpringBonesRequest, loadSpringBonesSuccess, setBones } from './actions'
...
describe('when handling the load spring bones request', () => {
  ...
  describe('and the item has a single representation', () => {
    ...
    it('should fetch the GLB once and dispatch setBones keyed by the representation hash', () => {
      return expectSaga(editorSaga as any)
        .provide([
  ...
  describe('and the item has both male and female representations', () => {
    ...
    it('should dispatch setBones once per unique GLB hash', () => {
      return expectSaga(editorSaga as any)
        .provide([
          [matchers.call.fn(fetchGlbBlob), blob],
          [matchers.call.fn(parseSpringBones), { bones: [] }]
        ...
        .put(clearSpringBones())
        .put(setBones(aHash, [], item.id))
        .put(setBones(otherHash, [], item.id))
        .put(loadSpringBonesSuccess(item.id))
        .dispatch(loadSpringBonesRequest(item))
        .silentRun()
    })
  ...
  describe('and metadata params are present for a spring bone', () => {
    ...
    it('should overlay metadata params onto the parsed spring bone', () => {
      ...
      return expectSaga(editorSaga as any)
        ...
        .put(clearSpringBones())
        .put(setBones(aHash, [{ name: 'springbone_hair', nodeId: 1, type: 'spring', children: [], params: metadataParams }], item.id))
        .put(loadSpringBonesSuccess(item.id))
        .dispatch(loadSpringBonesRequest(item))
        .silentRun()
    })
  ...
  describe('and a single GLB is shared across male and female representations', () => {
    ...
    it('should resolve metadata params once for the shared hash and dispatch a single setBones', () => {
      ...
      return expectSaga(editorSaga as any)
        ...
        .put(clearSpringBones())
        .put(setBones(aHash, [{ name: 'Hair_springBone', nodeId: 1, type: 'spring', children: [], params: metadataParams }], item.id))
        .put(loadSpringBonesSuccess(item.id))
        .dispatch(loadSpringBonesRequest(item))
        .silentRun()
    })
  ...
  describe('and the per-shape GLB fetch fails', () => {
    ...
    it('should swallow the per-hash error and still dispatch loadSpringBonesSuccess', () => {
      return expectSaga(editorSaga as any)
        .provide([[matchers.call.fn(fetchGlbBlob), Promise.reject(new Error('boom'))]])
        .put(clearSpringBones())
        .not.put(setBones(aHash, expect.anything(), item.id))
        .put(loadSpringBonesSuccess(item.id))
        .dispatch(loadSpringBonesRequest(item))
        .silentRun()
    })
  ...
  describe('and an unexpected error happens during orchestration', () => {
    ...
    beforeEach(() => {
      // Force a throw inside the saga body by making the item lack `data` entirely —
      // `isWearable` will read .type successfully but `item.data.representations` will throw.
      item = {
        ...mockedItem,
        contents: { 'anItemContent.glb': aHash },
    ...
    it('should dispatch loadSpringBonesFailure with the error message', () => {
      return expectSaga(editorSaga as any)
        .put(clearSpringBones())
        .put.like({ action: { type: loadSpringBonesFailure(item.id, '').type, payload: { itemId: item.id } } })
        .dispatch(loadSpringBonesRequest(item))
        .silentRun()
    })
  })
