...
import { getIsDCLControllerV2Enabled, getIsDeployToWorldsEnabled, getIsMaintenanceEnabled } from './selectors'
...
describe('when getting if dcl controller v2 is enabled', () => {
  describe('when getIsFeatureEnabled returns true', () => {
    beforeEach(() => {
      mockGetIsFeatureEnabled.mockReturnValueOnce(true)
    })
    ...
    it('should return true', () => {
      const state: RootState = {} as any
      const result = getIsDCLControllerV2Enabled(state)
