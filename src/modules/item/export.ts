...
import { Item, IMAGE_PATH, THUMBNAIL_PATH, ItemType, EntityHashingType, isEmoteItemType, VIDEO_PATH, isSocialEmote } from './types'
...
function buildADR287EmoteEntityMetadata(collection: Collection, item: Item<ItemType.EMOTE>): any {
  if (!collection.contractAddress || !item.tokenId) {
    throw new Error('You need the collection and item to be published')
  }

  // The order of the metadata properties can't be changed. Changing it will result in a different content hash.
  const catalystItem: any = {
    id: buildCatalystItemURN(collection.contractAddress, item.tokenId),
    name: item.name,
    description: item.description,
    collectionAddress: collection.contractAddress,
...
export async function buildItemEntity(
  ...
  if (isEmote) {
    metadata = isSocialEmote(item.data) ? buildADR287EmoteEntityMetadata(collection, item) : buildADR74EmoteEntityMetadata(collection, item)
