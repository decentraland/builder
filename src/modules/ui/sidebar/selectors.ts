...
export const getCategories = createSelector<RootState, string | null, string, AssetPackState['data'], AssetState['data'], Category[]>(
  ...
  (selectedAssetPackId, search, assetPacks, assets) => {
    ...
    const filteredAssetIds = Object.keys(assets)
      .filter(assetId => !search || isSearchResult(assets[assetId], search))
