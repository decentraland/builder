 ...
 * `lib/mana-convert.ts`) and the marketplace webapp's own `lib/credits` on purpose: a price that
 * disagrees between the apps is the bug this module exists to close. Deliberately duplicated rather
 * than shared as a dependency — none of these apps depend on each other.
 */
