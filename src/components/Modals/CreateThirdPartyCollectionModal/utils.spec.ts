...
describe('when checking if a network is supported', () => {
  describe.each([
    [ContractNetwork.MAINNET, true],
    [ContractNetwork.MATIC, true],
    [ContractNetwork.AMOY, true],
    [ContractNetwork.SEPOLIA, true],
    [ContractNetwork.BASE_MAINNET, false],
    [ContractNetwork.BASE_SEPOLIA, false],
    [ContractNetwork.APE_MAINNET, false],
    [ContractNetwork.APE_CALDERA, false],
    [ContractNetwork.MONAD_MAINNET, false],
    [ContractNetwork.MONAD_TESTNET, false]
  ])('and the network is %s', (network, isSupported) => {
    it(`should return ${isSupported}`, () => {
      expect(isSupportedNetwork(network)).toBe(isSupported)
    })
  })
})
