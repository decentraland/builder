...
import { createItemReducer, createItemActions } from './CreateSingleItemModal.reducer'
import {
  Props,
  State,
  CreateItemView,
...
export const CreateSingleItemModal: React.FC<Props> = props => {
  ...
  const handleDropAccepted = useCallback(
    (acceptedFileProps: AcceptedFileProps) => {
      ...
      dispatch(createItemActions.setBodyShape(bodyShape || state.bodyShape || BodyShapeType.MALE))
  ...
  const handleRarityChange = useCallback((_event: React.SyntheticEvent<HTMLElement, Event>, { value }: DropdownProps) => {
    const rarity = value as Rarity
    dispatch(createItemActions.setRarity(rarity))
  }, [])

  const handlePlayModeChange = useCallback((_event: React.SyntheticEvent<HTMLElement, Event>, { value }: DropdownProps) => {
    const playMode = value as EmotePlayMode
    dispatch(createItemActions.setPlayMode(playMode))
  }, [])
  ...
  const handleFileLoad = useCallback(async () => {
    ...
    // dispatch(createItemActions.setPreviewController(controller))
  ...
  const handleUploadVideoGoBack = useCallback(() => {
    ...
    dispatch(createItemActions.resetState(stateReset))
  }, [state])
  ...
  }, [collection, error, metadata, state, handleSubmit, renderModalTitle, isDisabled, isLoading, onClose])
  ...
  const handleOnScreenshotTaken = useCallback(
    async (screenshot: string) => {
      ...
      if (item && itemSortedContents) {
        ...
        // Determine the target view based on fromView
        let targetView = state.view
        if (state.fromView === CreateItemView.DETAILS) {
          targetView = CreateItemView.DETAILS
        } else {
          // Call handleSubmit and let it handle the view change
          void handleSubmit()
        }
        ...
        dispatch(createItemActions.setView(targetView))
      } else {
        // Determine the target view based on fromView
        let targetView = state.view
        if (state.fromView === CreateItemView.DETAILS) {
  ...
  const renderThumbnailView = useCallback(() => {
    ...
    return (
      <EditThumbnailStep
        ...
        onBack={() => dispatch(createItemActions.setView(CreateItemView.DETAILS))}
